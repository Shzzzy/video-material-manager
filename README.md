...
### 团队协作（无账户体系）
- **邀请码加入**：核心管理员生成邀请码，成员输入邀请码 + 昵称即可加入，无需注册
- **核心管理员**：首次使用管理员激活码的人成为管理员；管理员可管理邀请码（生成/拉黑）、成员（移出）、标签体系、扫描文件夹与系统设置
...
首次启动后端会在控制台打印**管理员激活码**（一次性，形如 `ADMIN-XXXX`），
打开前端页面输入激活码 + 昵称即可成为核心管理员；
之后在顶栏成员标识（金色）→ 管理工作台 → 生成邀请码分发给团队成员。
